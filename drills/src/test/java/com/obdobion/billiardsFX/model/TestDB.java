...
import com.obdobion.billiardsFX.model.impl.DrillMasterDbImpl;
...
public class TestDB
{
    ...
    public void saveAndReadFromSaveStore() throws Exception
    {
        ...
        final SaveStoreFilter filter = DrillMaster.saveDrillToSaveStore(
                JUnitDrillMasterImpl.J_UNIT_SESSION, drill);
        final List<IDrill> drills = DrillMaster.createDrillFromSaveStore(
                JUnitDrillMasterImpl.J_UNIT_SESSION, filter, 1);

        Assert.assertEquals(1, drills.size());
    ...
    {
        final DrillMasterDbImpl impl = new DrillMasterDbImpl();
        previousDrillSergeant = DrillMaster.setInstance(impl);
        myDS = new JUnitDrillMasterImpl();
        myDS.addObserver(new JUnitDrillMasterImpl());
        impl.getDrillPackageManager().add(new SoloGames());
    }
